...
public class LineChartViewForYinJi extends View{
    ...
    private float mYAxis = 10f;// x 轴高度，表示 x 轴的起始高度
    private float mYAxisStartHeight = 10f;
    ...
    private void drawLineAndPoints(Canvas canvas) {
        canvas.drawLine(mStartX, mYAxis, mViewWidth - mStartX, mYAxis, mLinePaint);
        if (mList != null && mList.size() > 0) {
            for (int i = 0; i < mList.get(mList.size() - 1).getPeople(); i++) {
                canvas.drawCircle(mPointSpacingWidth * (i + 1) + mStartX * 2, mYAxis, 5f, mTextPaint);
            }
        }
    }
    ...
    private void drawPriceAndRect(Canvas canvas) {
        ...
        float pointPriceY = mYAxis / 2;// 中心点为 line 向上的一半
