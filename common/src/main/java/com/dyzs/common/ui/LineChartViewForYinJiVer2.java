...
public class LineChartViewForYinJiVer2 extends View {
    ...
    public LineChartViewForYinJiVer2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        ta.recycle();
        mPerLineDuration = mPerLineDuration < 80 ? 80 : mPerLineDuration;
...
//        playLineAnimation();
    ...
    private void initPaintAndPath() {
        ...
        mDottedPaint.setColor(ContextCompat.getColor(getContext(), R.color.gray_white));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mXAxisTerminal = widthPixels - widthPixels * 0.1f;
        ...
        mXAxisTerminal = mViewWidth - widthPixels * 0.1f;
    ...
    private void drawXAxisAndPoints(Canvas canvas) {
        ...
        for (int i = 0; i <= getXAxisDisplayCount(); i++) {
            float fx = mPointSpacingWidth * i + mXAxisStart * 1f;
            float fy = mYAxisTerminal;
            // drawDottedLine(canvas, fx, mYAxisStart, fx, fy);

            canvas.drawCircle(fx, fy, 10f, mXYAxisPointsPaint);
    ...
    private void drawDottedLine(Canvas canvas, float startX, float startY, float stopX, float stopY) {
        mDottedPaint.setPathEffect(new DashPathEffect(new float[]{5, 5}, 5));
        Path mPath = new Path();
        mPath.reset();
    ...
    private void drawPriceAndRect(Canvas canvas) {
        ...
        for (int i = 0; i < mListPoints.size(); i ++) {
            ...
            mTextPaint.setTextSize(dp2Px(10));//改变x轴坐标的字体大小
            canvas.drawText(
                    text,
    ...
    private void handleActionUp(MotionEvent event) {
        ...
        if (upTime - mTouchDownTime < 100) {
            ...
            for (int i = 0; i < mListPoints.size(); i ++) {
                ...
                t = 0;//mListPoints.get(i).y - touchLimit;
                r = mListPoints.get(i).x + touchLimit;
                b = getMeasuredHeight();// mListPoints.get(i).y + touchLimit;
    ...
    public void setData(ArrayList<ViewItem> listData) {
        ...
        for (int i = 0; i < mListData.size(); i++) {
            ...
            mSelection = mSelection > 0 ? mSelection - 1 : mSelection;
        }
        if (temp.size() > 0) {
     ...
     * 30拆解为 3 * 5 * 2, 主要是计算 3*5 的公倍数, 这样显示在 y 轴的数字时候看起来比较整齐
     * @param listMax
     */
    private void resetPeakValue (float listMax) {
        mYAxisPeakValue = Math.round(listMax + 0.5f);
